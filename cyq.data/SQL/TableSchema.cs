...
namespace CYQ.Data.SQL
{
                ...
                else
                {
                    ...
                    switch (dalType)
                    {
                        ...
                        case DalType.Sybase:
                            ...
                            {
                                sql = GetMySqlColumns(helper.DataBase, SqlFormat.NotKeyword(tableName));
                            }
    ...
    internal partial class TableSchema
    {
        ...
        internal static string GetMySqlColumns(string dbName,string tableName)
        {
            return string.Format(@"SELECT DISTINCT s1.ORDINAL_POSITION,s1.COLUMN_NAME as ColumnName,case DATA_TYPE when 'int' then 10 when 'date' then 10 when 'time' then 8  when 'datetime' then 23 when 'year' then 4
                    ...
                    where s1.TABLE_SCHEMA='{0}' and  s1.TABLE_NAME='{1}' order by s1.ORDINAL_POSITION", dbName, tableName);
        }
